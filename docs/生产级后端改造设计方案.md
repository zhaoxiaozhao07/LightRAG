...
> - ⏭️ 仍未执行（按约定推迟）：共享图谱删除精确策略（source attribution / reference count / 保守重建的可配置 API 层）、生产 metadata DB 迁移、用户隔离 / RBAC / 审计 / 配额 / 限流。（全 KB rebuild 与可选 durable worker 已落地。）
...
当前实现仅支持以下三个选项（query param / 请求体字段）：
...
> 设想中的 `delete_graph_orphans` 与 `strategy`（`safe` / `rebuild_doc_scope` / `rebuild_kb`）尚未在 API 层暴露，下表策略为目标设计。底层 `LightRAG.adelete_by_doc_id` 已内建 source-attribution（按剩余来源判定）+ 共享实体保守重建，等价于 `safe` 的图谱保护语义；KB 层暂未提供可配置策略开关。
...
- 🟡 Worker 解析中崩溃恢复：可选 durable worker 的原子认领、`:retry` 自动消费、重启后保留 parse/build_kg/reindex 单文档 queued job、真实 `build_parse_executor`/`build_build_kg_executor` 重驱动均已加测试；仍待覆盖真实 asyncio 轮询 loop 生命周期与 live 服务故障。
- ⏭️ 删除中失败，重试后最终一致（delete/replace 重启续跑仍未纳入 durable worker）。
...
下一步执行建议：MVP 主链路、Phase 4C document replace、全 KB rebuild、可选 durable worker 均已具备，`:reindex`/`force_*` 已修复为真正重跑 LightRAG pipeline，后续应继续收敛共享图谱删除精度和生产 metadata DB migration。推荐顺序为：

1. 补齐共享图谱删除策略：source attribution / reference count / conservative rebuild 三选一或分层组合（当前删除直接复用 `LightRAG.adelete_by_doc_id` 的内建 source-attribution + 保守重建，尚未在 API 层暴露 `strategy`/`delete_graph_orphans` 可配置选项）。
2. ✅ 已实现 `/kbs/{kb_id}:rebuild`，枚举全部可构建文档批量 force reindex（空 KB no-op）。
3. 迁移生产 metadata DB schema/migration，并保留 SQLite 测试覆盖。
4. ✅ 把 queued/running job 从同步背景任务推进到可选 durable worker（`LIGHTRAG_KB_JOB_WORKER`），补 `:retry` 自动消费与重启后 parse/build/reindex 单文档 queued job 自动续跑（真实执行器已加测试）；仍待覆盖 replace/sync/delete 等依赖请求级上传字节的任务恢复。
