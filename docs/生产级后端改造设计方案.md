...
> - ⏭️ 仍未执行（按约定推迟）：生产 metadata DB 迁移、用户隔离 / RBAC / 审计 / 配额 / 限流。（全 KB rebuild、可选 durable worker、共享图谱删除策略 API（`strategy` + `safe`/`rebuild_doc_scope`/`rebuild_kb`）、运行 job 协作式取消检查点、单文档 delete 的 durable 恢复均已落地。）
...
> 当前状态：✅ `jobs` 已临时落地 SQLite，由 `JobService` 管理；支持 upload/parse/build/reindex/delete/replace/sync 等 job 创建、列表/详情、wait、cancel、retry、running_jobs 汇总、基础状态流转、批量 per-item result、active conflict、幂等 key 复用与冲突。✅ 可选 durable worker（`LIGHTRAG_KB_JOB_WORKER=true`）已落地：原子认领（`queued→running` 单赢 CAS，且**仅认领 `document_id IS NOT NULL` 的单文档 job**）并自动消费 `parse`/`build_kg`/`reindex`/单文档 `delete` 的 queued job（含 `:retry` 重置后），重启时保留这些可恢复类型的单文档 queued job 交由 worker 续跑；聚合任务（`document_id=null` 的 batch-parse/build/reindex/delete、多文件 upload/texts 的 auto_parse、sync）不被认领、孤儿恢复直接标 `failed`；宽限窗口避免抢占新建任务的 in-process 执行；running parse/build job 已具备阶段边界协作式取消检查点。🟡 仍待：`replace`/批量 `sync`/批量 `delete`/聚合 auto_parse 等依赖请求级上传字节或一次性上下文的任务恢复、running job await 内强制中断和 dead-letter 策略。
