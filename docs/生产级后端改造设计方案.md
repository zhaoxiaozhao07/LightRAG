...
> - ⏭️ **仍按约定推迟**：`replace`/批量 `sync` 的请求字节持久化与 durable executor（需改造上传写路径，未做，代码与文档已如实标注其不可恢复）；运行中单次长 await 的强制中断；共享图谱精确子图局部重建；live MinerU/跨 backend 真集成测试；生产 metadata DB 迁移。
...
> - ⏭️ 仍未执行（按约定推迟）：生产 metadata DB 迁移、用户隔离 / RBAC / 审计 / 配额 / 限流。（全 KB rebuild、可选 durable worker、共享图谱删除策略 API（`strategy` + `safe`/`rebuild_doc_scope`/`rebuild_kb`）、运行 job 协作式取消检查点、单文档和批量 delete 的 durable 恢复、parser_config runtime 默认值、细粒度 parse artifact 记录均已落地。）
...
> 当前状态：✅ `kb_config_versions` 已在 SQLite metadata schema 中落地，并已暴露 ConfigVersionService/API：创建、列表、详情、`:activate`、`:diff` 均已实现并测试。激活会写入 KB 的 `active_config_version_id` 并 discard 缓存实例；后续 KB LightRAG 重建会应用已支持的 chunk / embedding / query / extraction runtime 字段（`extraction_config` 的 `language`、`entity_types`/`entity_types_guidance`、`entity_type_prompt_file`、抽取 caps 已接入 `addon_params` 与构造参数，并纳入 `index_hash`）。`parser_config.engine/process_options` 已接入 parse runtime 默认值，并在配置创建时校验/规范化；仍未接入 runtime 的配置字段（parser 服务实例级参数、`llm_role_config`、`storage_config`）仍仅作为 metadata / diff 信息保存。
...
> 当前状态：✅ SQLite schema 已创建 `document_artifacts` 表，并由 parse 成功路径写入 `original`、`sidecar`、`blocks`、`raw_dir` 以及 `markdown/content_list/middle_json/model_json/image/layout_pdf` 等细粒度 artifact 记录；已提供 artifact list/detail API、文件型 artifact download API，以及目录型 artifact zip 下载。⏭️ 对象存储签名 URL 待后续完善。
...
> 当前状态：✅ `jobs` 已临时落地 SQLite，由 `JobService` 管理；支持 upload/parse/build/reindex/delete/replace/sync 等 job 创建、列表/详情、wait、cancel、retry、running_jobs 汇总、基础状态流转、批量 per-item result、active conflict、幂等 key 复用与冲突。✅ 可选 durable worker（`LIGHTRAG_KB_JOB_WORKER=true`）已落地：原子认领（`queued→running` 单赢 CAS）并自动消费 `parse`/`build_kg`/`reindex`/单文档 `delete` 的 queued job（含 `:retry` 重置后），同时支持 `documents:batch-delete` 聚合 `delete` job（`document_id=null`，从 payload 恢复 `document_ids` 与删除选项）。重启时保留这些可恢复类型的 queued job 交由 worker 续跑；其他聚合任务（batch-parse/build/reindex、多文件 upload/texts 的 auto_parse、sync）不被认领、孤儿恢复直接标 `failed`；宽限窗口避免抢占新建任务的 in-process 执行；running parse/build job 已具备阶段边界协作式取消检查点。🟡 仍待：`replace`/批量 `sync`/聚合 auto_parse 等依赖请求级上传字节或一次性上下文的任务恢复、running job await 内强制中断和 dead-letter 策略。
...
- ✅ 多文件上传，每个文件独立创建 document 记录；`auto_parse=false` 仅持久化 source 文件和 SQLite metadata（不触发 pipeline），`auto_parse=true` 则在后台逐个执行解析（`auto_index=true` 串联 KG/index 到 `ready`）。
...
- ✅ 文件型 artifact 下载：`original/blocks` 等文件 artifact 可通过 `:download` 获取；下载前按 `inputs/<workspace>/<document_id>` 做路径 containment 校验，跨 KB、路径逃逸、缺失文件返回错误，目录型 `sidecar/raw_dir` 以流式 zip 返回（512MB 上限，超限 413）。
...
下一步执行建议：MVP 主链路、Phase 4C document replace、全 KB rebuild、可选 durable worker、共享图谱删除策略 API、运行 job 协作式取消、单文档 delete durable 恢复均已具备，`:reindex`/`force_*` 已修复为真正重跑 LightRAG pipeline，后续应继续收敛生产 metadata DB migration。推荐顺序为：
