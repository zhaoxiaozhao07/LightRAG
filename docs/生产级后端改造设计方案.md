...
> - ✅ **上传源文件与解析产物可持久化到 MinIO/S3**。新增 `ObjectStorage` 抽象与 `S3ObjectStorage`，通过 `LIGHTRAG_OBJECT_STORAGE=minio|s3` 启用。上传/replace 会先写 `INPUT_DIR/<workspace>/<document_id>/...` 本地 cache，再同步对象存储并记录 `metadata.source_object_uri`；parse artifact 记录 `metadata.object_uri` 或 `metadata.object_prefix_uri`；下载时本地 cache 缺失会从对象存储 restore；delete/replace/hard-delete 会同步删除对象。
...
> - ⏭️ **仍按约定推迟**：`replace`/批量 `sync` 的请求字节 durable executor（source bytes 已可进入对象存储，但 worker 恢复仍缺少请求上下文和执行器注册）；运行中单次长 await 的强制中断；共享图谱精确子图局部重建；live MinerU 真集成测试；PostgreSQL/MinIO 生产环境迁移脚本与数据回填工具。
...
> - ⏭️ **仍按约定推迟**：批量 `sync` durable 续跑（一次请求多文件，需为*所有* item 落盘 per-item 字节并定义部分批次续跑语义，属更大改造）；多文件 `upload`（无 auto_parse 时无解析工作）。
...
> - ⏭️ 仍未执行（按约定推迟）：JSON/SQLite → PostgreSQL 迁移/回填脚本、用户隔离 / RBAC / 审计 / 配额 / 限流。（全 KB rebuild、可选 durable worker、共享图谱删除策略 API（`strategy` + `safe`/`rebuild_doc_scope`/`rebuild_kb`）、运行 job 协作式取消检查点、单文档和批量 delete 的 durable 恢复、parser_config runtime 默认值、细粒度 parse artifact 记录、PostgreSQL 可选后端和 MinIO/S3 对象存储均已落地。）
...
> 当前状态：✅ `jobs` 已由 `JobService` 管理；local 模式落地 SQLite，PostgreSQL 模式落地 `kb_jobs` JSONB 表。支持 upload/parse/build/reindex/delete/replace/sync/clear_kb 等 job 创建、列表/详情、wait、cancel、retry、running_jobs 汇总、基础状态流转、批量 per-item result、active conflict、幂等 key 复用与冲突。✅ 可选 durable worker（`LIGHTRAG_KB_JOB_WORKER=true`）已落地：原子认领（`queued→running` 单赢 CAS）并自动消费 `parse`/`build_kg`/`reindex`/单文档 `delete` 的 queued job（含 `:retry` 重置后），同时支持 `documents:batch-delete` 聚合 `delete` job（`document_id=null`，从 payload 恢复 `document_ids` 与删除选项）和 `clear_kb` 硬删除 job。重启时保留这些可恢复类型的 queued job 交由 worker 续跑；running parse/build job 已具备阶段边界协作式取消检查点，且 parse 阶段进一步支持 await 内强制中断（在途 MinerU/Docling 解析可被 `:cancel` 立即打断，因解析幂等可安全重跑）。单文档 `replace` 现已 durable：替换字节在 claim 时落盘 staging，worker 凭 staged 字节重建并续跑。🟡 仍待：批量 `sync`/多文件 `upload` 聚合 auto_parse 等依赖请求级上传字节或一次性上下文的任务恢复（`sync` 需为所有 item 落盘 per-item 字节并定义部分批次续跑语义）。
...
- 🟡 `knowledge_bases` 表：JSON metadata 默认实现和 PostgreSQL `kb_catalog` 后端均已落地；仍需补 JSON→PostgreSQL 数据迁移/回填脚本。
...
| 方法 | 路径 | 说明 | 当前状态 |
|---|---|---|---|
| `POST` | `/api/v1/kbs/{kb_id}/documents:upload` | 多文件上传，可选择自动解析/自动索引。 | ✅ 已实现 / 已测试；`auto_parse=true` 创建 `parse` 聚合 job 并由 in-process 后台任务实际逐个解析（`auto_index=true` 串联构建到 `ready`）；该聚合任务不被 durable worker 认领 |
...
- 🟡 Worker 解析中崩溃恢复：可选 durable worker 的原子认领、`:retry` 自动消费、重启后保留 parse/build_kg/reindex/单文档 delete 的 queued job、真实 `build_parse_executor`/`build_build_kg_executor`/`build_delete_executor` 重驱动、后台轮询 loop 生命周期（start/_run_loop/stop）均已加测试；仍待覆盖 live 服务故障。
- 🟡 删除中失败，重试后最终一致：单文档 delete 已纳入 durable worker（重启重置 `deleting→delete_failed` 后 worker 重新 claim 续跑，已测试）；批量 `delete`、`replace`、`sync` 重启续跑仍未纳入。
...
1. 🟡 metadata DB schema：KB、config version、document、artifact、job 的 PostgreSQL 后端已实现（JSONB record + projection/index）；仍需 JSON/SQLite → PostgreSQL 迁移/回填脚本与生产 rollout 指南。
...
下一步执行建议：MVP 主链路、Phase 4C document replace、全 KB rebuild、可选 durable worker、共享图谱删除策略 API、运行 job 协作式取消、单文档 delete durable 恢复、PostgreSQL KB metadata 后端和 MinIO/S3 source/artifact 持久化均已具备，`:reindex`/`force_*` 已修复为真正重跑 LightRAG pipeline，后续应继续收敛迁移工具与生产集成验证。推荐顺序为：
...
3. 补 JSON/SQLite → PostgreSQL metadata 迁移/回填脚本、MinIO bucket/prefix 备份恢复 runbook，并保留 SQLite/PostgreSQL 双后端测试覆盖。
4. ✅ 把 queued/running job 从同步背景任务推进到可选 durable worker（`LIGHTRAG_KB_JOB_WORKER`），补 `:retry` 自动消费与重启后 parse/build/reindex/单文档 delete 的 queued job 自动续跑（真实执行器 + 后台 loop 生命周期已加测试）；仍待覆盖 replace/sync/批量 delete 等依赖请求级上传字节或一次性上下文的任务恢复。
