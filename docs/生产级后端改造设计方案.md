...
```http
POST   /api/v1/kbs/{kb_id}/documents:upload          ✅ 已实现 / 已测试（auto_parse 后台实际执行解析；auto_index 串联构建）
POST   /api/v1/kbs/{kb_id}/documents:sync            ✅ source_key 增量同步 + parse/build 编排已实现 / 已测试
POST   /api/v1/kbs/{kb_id}/documents:texts           ✅ 已实现 / 已测试（auto_parse 后台实际执行解析；auto_index 串联构建）
...
- ✅ 多文件上传，每个文件独立创建 document 记录；`auto_parse=false` 仅持久化 source 文件和 metadata（local SQLite 或 PostgreSQL），`auto_parse=true` 则在后台逐个执行解析（`auto_index=true` 串联 KG/index 到 `ready`）；启用对象存储时 source 同步写 MinIO/S3 并记录 `source_object_uri`。
...
- ✅ 解析 job 持久化，不依赖内存 queue 作为唯一状态；`auto_parse=true` 时创建 `job_type=parse` 聚合 queued job 并由 in-process 后台任务逐个执行解析（`auto_index=true` 串联构建），`auto_parse=false` 时创建 succeeded upload job；显式 `:parse` 会创建单文档 parse job 并在后台流转到 `running/succeeded/failed`。
...
| 方法 | 路径 | 说明 | 当前状态 |
|---|---|---|---|
| `POST` | `/api/v1/kbs/{kb_id}/documents:upload` | 多文件上传，可选择自动解析/自动索引。 | ✅ 已实现 / 已测试；`auto_parse=true` 创建 `parse` 聚合 job 并由 in-process 后台任务实际逐个解析（`auto_index=true` 串联构建到 `ready`）；启用 `LIGHTRAG_KB_JOB_WORKER=true` 时，queued 且超过 grace window 的 upload auto_parse 聚合 job 可被 durable worker 凭 `document_ids` 续跑；`auto_parse=false` 不产生需恢复的解析工作 |
