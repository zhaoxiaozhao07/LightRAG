...
> 当前状态：🟡 第一版 SQLite metadata schema 已创建 `kb_config_versions` 表，用于后续配置版本切片；当前尚未暴露 ConfigVersionService/API，也未把 KB 的 `active_config_version_id` 自动绑定到该表。
...
| `index_hash` | chunk/embedding/extraction/KG 构建相关 hash。 |
| `query_hash` | 查询默认参数 hash。 |
| `created_at/created_by` | 审计预留。 |

### 4.3 documents

> 当前状态：✅ 第一版 `documents` 已临时落地 SQLite（非向量控制面元数据），由 `DocumentLifecycleService` 写入；支持 upload/text source、source hash、source_uri、status、metadata、KB 隔离、列表、详情、`source_name` 过滤和 `PATCH metadata/enabled/archived`。✅ `PATCH metadata` 会拒绝覆盖 `batch_id/pending_parse_job_id/current_parse_job_id/process_options` 等内部控制面保留 key。✅ 单文档/批量 parse 成功后会填充 `lightrag_doc_id`、`parser_hash` 并同步 `parse_queued/parsing/parsed/parse_failed` 状态；active parse claim 在 SQLite 写事务中完成，避免同一文档重复解析。⏭️ `DELETE/replace/enable/disable` 独立动作、`index_hash`、chunk/entity/relation 统计由后续 delete/index/KG 阶段补齐。
...
> 当前状态：✅ SQLite schema 已创建 `document_artifacts` 表，并由 parse 成功路径写入 `original`、`sidecar`、`blocks`、`raw_dir` 等首版 artifact 记录；已提供 artifact list/detail API 和文件型 artifact download API。⏭️ markdown/content_list/middle_json/images/layout_pdf 等更细粒度 artifact 类型、目录型 artifact zip/stream 和对象存储签名 URL 待后续完善。
...
> 当前状态：✅ 第一版 `jobs` 已临时落地 SQLite，由 `JobService` 管理；支持 upload/parse queued job 创建、列表/详情、running jobs 汇总、基础状态流转、单文档 parse 失败、批量 parse partial failure、active parse conflict 计数和结果持久化测试。✅ Phase 2 创建型路径已补 idempotency key 复用：文本导入、单文档 parse、批量 parse 在 SQLite 写事务内按 `(kb_id, job_type, idempotency_key)` get-or-create，重复请求返回既有 job/batch，不重复创建 job 或重复执行 parser；同 key 但请求指纹不同返回 409。⏭️ cancel/retry API、durable worker 恢复、失败重试策略仍待后续阶段。
...
DELETE /api/v1/kbs/{kb_id}/documents/{doc_id}        ⏭️ 待实现
POST   /api/v1/kbs/{kb_id}/documents:batch-delete    ⏭️ 待实现
...
- ✅ active parse conflict：显式单文档 parse 遇到已有 `parse_queued/parsing` 时返回 409 并将新建 job 标记 failed；批量 parse 将 active 文档作为 per-item failure，且只执行 claim 成功的文档。
...
| 文档 metadata / 上传 / 文本导入 | SQLite `documents/jobs/artifacts/config_versions` 临时 schema；上传/文本导入、列表/详情、状态分页、`source_name` 过滤、`PATCH metadata/enabled/archived`、保留 metadata key 拒绝、KB scoped metadata 查询；扩展名、大小、文件数、文本/metadata 限制；同名文件独立目录和独占创建；文本导入 idempotency key 事务内复用。 | 文档 `DELETE`、`batch-delete`、`replace`、`enable/disable` 独立动作；URL/scan/import 类型接入。 | 两个 KB 同名文件上传的完整端到端隔离只覆盖到 metadata/artifact 层，KB 级 query 隔离待 Phase 5。 |
...
| `index_hash` | chunk、embedding、抽取 prompt、KG 构建参数 | 复用解析产物，重新索引。 |
...
| `PATCH` | `/api/v1/kbs/{kb_id}` | 更新名称、描述、默认配置指针等。 | ✅ 已实现 / 已测试 |
| `DELETE` | `/api/v1/kbs/{kb_id}` | 删除知识库，建议异步。 | 🟡 soft delete 已实现 / 已测试；异步 hard delete 待 Phase 4 |
...
| `DELETE` | `/api/v1/kbs/{kb_id}/documents/{doc_id}` | 单文档异步删除。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/documents:batch-delete` | 批量异步删除。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/documents/{doc_id}:replace` | 替换文件并触发增量重建。 | ⏭️ 待实现 |
...
| 方法 | 路径 | 说明 | 当前状态 |
|---|---|---|---|
| `POST` | `/api/v1/kbs/{kb_id}/documents/{doc_id}:build-kg` | 单文档实体关系抽取和 KG 构建。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/documents:batch-build-kg` | 批量 KG 构建。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/documents/{doc_id}:reindex` | 单文档重建索引。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/documents:batch-reindex` | 批量重建索引。 | ⏭️ 待实现 |
...
| 方法 | 路径 | 说明 | 当前状态 |
|---|---|---|---|
| `POST` | `/api/v1/kbs/{kb_id}/query` | 非流式问答。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/query/stream` | 流式问答。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/retrieve` | 只检索，不生成答案。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/query/data` | 返回结构化检索数据。 | ⏭️ 待实现 |
...
| `POST` | `/api/v1/kbs/{kb_id}/jobs/{job_id}:cancel` | 取消任务。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/jobs/{job_id}:retry` | 重试任务。 | ⏭️ 待实现 |

### 7.7 Config APIs

| 方法 | 路径 | 说明 | 当前状态 |
|---|---|---|---|
| `GET` | `/api/v1/kbs/{kb_id}/configs` | 配置版本列表。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/configs` | 创建新配置版本。 | ⏭️ 待实现 |
| `GET` | `/api/v1/kbs/{kb_id}/configs/{version_id}` | 配置版本详情。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/configs/{version_id}:activate` | 激活配置版本。 | ⏭️ 待实现 |
| `POST` | `/api/v1/kbs/{kb_id}/configs/{version_id}:diff` | 比较配置变化和重建影响。 | ⏭️ 待实现 |
...
| `committing` | 正在写入最终存储。 |
| `ready` | 可查询。 |
| `disabled` | 不参与查询，但存储仍保留。 |
| `archived` | 归档，不默认展示/查询。 |
...
- 🟡 创建两个 KB，分别上传/导入文档并确认 metadata/artifact 访问互不影响已部分覆盖；KB 级查询隔离待 Phase 5。
- ⏭️ KB A 查询不返回 KB B 内容。
- ✅ 单文件 MinerU parse 成功并保存 artifacts（测试使用 fake parser，不依赖 live MinerU）。
- ✅ 批量 parse 部分失败时 per-item 状态正确；active parse conflict 已覆盖。
- ⏭️ 新文件增量上传后只处理新增文档（依赖 KG/index 阶段）。
- ⏭️ 删除文档后查询不再返回该文档引用。
- ⏭️ 配置变更后 diff 正确提示 reparse/reindex。
...
5. 🟡 实现持久化 job 查询、取消、重试。当前 job 创建/列表/详情/状态流转骨架已测试；cancel/retry API 待实现。
6. ✅ 实现单文档 parse job、批量 parse 聚合 job、首版 artifact 保存和文件型 artifact download（parse-only，未触发 KG/index）。
7. ⏭️ 实现 KG build/index job。
8. ⏭️ 实现 KB 级 query。
9. ⏭️ 实现 batch delete 与删除一致性策略。
10. ⏭️ 实现 config version、hash diff、按需 reparse/reindex。
11. ⏭️ 用户隔离、RBAC、审计、配额、限流：按当前阶段约束暂不执行，仅保留字段/API 边界。

下一步执行建议：Phase 2 的 metadata/control-plane 骨架、单文档/批量 parse-only 路径和文件型 artifact download 已经具备，后续应继续补齐重试能力，再进入 KG/index。推荐顺序为：

1. 补 job cancel/retry API，明确 parse job 重试时是否替换旧 artifacts。
2. 继续实现 `:build-kg` / `:reindex`，从已持久化 artifacts/sidecar 读取解析结果，避免重复调用 MinerU。
3. 实现 KB 级 query wrapper，并验证 KB A 查询不返回 KB B 内容。
4. 实现文档 delete/batch-delete/replace/enable/disable 与删除一致性策略。
5. 实现 config version service/API、hash diff、按需 reparse/reindex。
6. 最后再迁移生产 metadata DB schema/migration 和 durable worker/recovery 机制。
...
6. ⏭️ `POST /api/v1/kbs/{kb_id}/documents/{doc_id}:build-kg`
7. ⏭️ `POST /api/v1/kbs/{kb_id}/query`
8. ✅ `GET /api/v1/kbs/{kb_id}/jobs/{job_id}`
9. ⏭️ `DELETE /api/v1/kbs/{kb_id}/documents/{doc_id}`

当前额外已完成并测试：`GET /api/v1/kbs`、`PATCH /api/v1/kbs/{kb_id}`、`DELETE /api/v1/kbs/{kb_id}` soft delete、`GET /api/v1/kbs/{kb_id}/status`（含 SQLite running_jobs）、`POST /api/v1/kbs/{kb_id}/documents:texts`、`GET /api/v1/kbs/{kb_id}/documents/{doc_id}`、`POST /api/v1/kbs/{kb_id}/documents:batch-parse`、`GET /api/v1/kbs/{kb_id}/documents/{doc_id}/artifacts`、`GET /api/v1/kbs/{kb_id}/documents/{doc_id}/artifacts/{artifact_id}`、`GET /api/v1/kbs/{kb_id}/documents/{doc_id}/artifacts/{artifact_id}:download`、`GET /api/v1/kbs/{kb_id}/jobs`。

这 9 个接口完成后即可支撑：创建知识库、上传文件、MinerU 解析、知识图谱构建、查询、状态追踪和删除闭环。当前已经完成其中的 KB、上传/文档查询、parse-only、artifact、job 查询部分；`build-kg`、KB query 和 document delete 尚未完成，因此还不能声称形成完整 RAG 闭环。
