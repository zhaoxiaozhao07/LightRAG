...
> 当前状态：✅ `jobs` 已临时落地 SQLite，由 `JobService` 管理；支持 upload/parse/build/delete/replace/sync 等 job 创建、列表/详情、wait、cancel、retry、running_jobs 汇总、基础状态流转、批量 per-item result、active conflict、幂等 key 复用与冲突。✅ 可选 durable worker（`LIGHTRAG_KB_JOB_WORKER=true`）已落地：原子认领（`queued→running` 单赢 CAS）并自动消费 `parse`/`build_kg`/`reindex` 单文档 queued job（含 `:retry` 重置后），重启时保留这些可恢复类型的 queued job 交由 worker 续跑；宽限窗口避免抢占新建任务的 in-process 执行。🟡 仍待：`replace`/批量 `sync`/`delete` 等依赖请求级上传字节的任务恢复、running job cooperative cancellation 检查点和 dead-letter 策略。
