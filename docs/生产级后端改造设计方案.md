...
> - ⏭️ **仍按约定推迟**：`replace`/批量 `sync` 的请求字节 durable executor（source bytes 已可进入对象存储，但 worker 恢复仍缺少请求上下文和执行器注册）；运行中单次长 await 的强制中断；共享图谱精确子图局部重建；live MinerU/跨 backend 真集成测试；PostgreSQL/MinIO 生产环境迁移脚本与数据回填工具。
...
- ✅ PostgreSQL metadata 后端选择、source/artifact 对象存储 URI/prefix 持久化、对象存储 restore/download、delete/replace/hard-delete 对象清理均已补回归测试；live PostgreSQL/MinIO 仍需集成环境验证。
...
1. ✅ 已实现共享图谱删除策略 API：`strategy`（`safe`/`rebuild_doc_scope`/`rebuild_kb`）+ `delete_graph_orphans`，底层复用 `adelete_by_doc_id` 的 source-attribution + 保守重建，`rebuild_kb` 额外串联剩余文档保守全量 reindex。后续可深化为按受影响实体子图的精确局部重建。
...
这些接口已经支撑：创建知识库、上传文件、MinerU/Docling/native 解析、知识图谱构建、KB 级查询、状态追踪、删除和替换闭环。后续仍需补共享图谱删除精确策略深化、生产迁移/回填工具、live PostgreSQL/MinIO/MinerU 集成测试和 replace/sync durable 恢复，才能覆盖更强的生产恢复能力。
