...
|---|---|---|---|
| KB metadata / CRUD | `POST/GET/PATCH/DELETE /kbs`、`GET /kbs/{kb_id}/status`、workspace 无碰撞映射、deleted tombstone、PATCH omitted-vs-null、跨进程 metadata 写锁、registry single-flight/discard；默认 JSON catalog、PostgreSQL `kb_catalog` 后端、JSON/SQLite → PostgreSQL metadata 迁移/回填工具均已实现。 | 异步 hard delete / storage cleanup 调度；MinIO bucket/prefix 备份恢复 runbook。 | KB 删除期间与上传/解析/查询的完整并发保护；hard delete 对底层 KV/Vector/Graph/DocStatus/input/artifact/object-storage 的清理一致性仍需更大范围集成验证。 |
| 文档 metadata / 上传 / 文本导入 | local SQLite 与 PostgreSQL `documents/jobs/artifacts/config_versions` metadata store；上传/文本导入、列表/详情、状态分页、`source_name` 过滤、`PATCH metadata/enabled/archived`、`:enable/:disable` 独立动作、job-based `DELETE` / `batch-delete`、job-based `:replace`、保留 metadata key 拒绝、KB scoped metadata 查询；扩展名、大小、文件数、文本/metadata 限制；同名文件独立目录和独占创建；文本导入 / replace idempotency key 事务内复用；MinIO/S3 source 持久化与本地 cache fallback；JSON/SQLite → PostgreSQL metadata 迁移工具。 | URL/scan/import 类型接入。 | 两个 KB 同名文件上传的完整端到端隔离只覆盖到 metadata/artifact 层，KB 级 query 隔离待 Phase 5。 |
| Parse-only / artifacts | 单文档 `:parse`、批量 `documents:batch-parse` 聚合 job、job/document 状态流转、fake MinerU/native/docling 成功/失败、request-level `force_reparse`、artifact list/detail、文件型 download、目录型 zip download、对象存储 URI/prefix 持久化与 restore、跨 KB/缺失文件/路径逃逸。 | 对象存储签名 URL；更细粒度生产 artifact 浏览/预览。 | live MinerU 服务超时/不可用的集成测试；真实外部服务解析耗时/网络失败；服务重启后自动恢复 queued/running parse job。 |
