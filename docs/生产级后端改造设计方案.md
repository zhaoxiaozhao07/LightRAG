...
> 当前状态：✅ `documents` 控制面元数据由 `DocumentLifecycleService` 写入；local 模式临时落地 SQLite，PostgreSQL 模式落地 `kb_documents` JSONB 表。支持 upload/text source、source hash、source_uri、status、metadata、KB 隔离、列表、详情、`source_name` 过滤、`PATCH metadata/enabled/archived`、`:enable/:disable`、job-based `DELETE` / `batch-delete`、job-based `:replace`、`source_key` 增量 sync，以及 parse/build 后回填 `lightrag_doc_id`、`parser_hash`、`index_hash`、chunk/entity/relation 统计。✅ 若启用对象存储，source 文件会同时持久化到 MinIO/S3，`metadata.source_object_uri` 保存对象 URI；`source_uri` 继续指向 `INPUT_DIR/<workspace>/<document_id>/...` 本地 cache，以兼容 parser/build/download 的本地路径流程。✅ `enabled/archived` 已接入检索层过滤：KB query 在解析 `kb_id` 后把"可检索集合"（`enabled=true` 且 `archived=false` 且有 `lightrag_doc_id`）与 `filters.doc_ids` 取交集映射为 `QueryParam.ids` 白名单，过滤 naive/实体/关系三路 chunk 证据。
...
| 文档 metadata / 上传 / 文本导入 | local SQLite 与 PostgreSQL `documents/jobs/artifacts/config_versions` metadata store；上传/文本导入、列表/详情、状态分页、`source_name` 过滤、`PATCH metadata/enabled/archived`、`:enable/:disable`、job-based `DELETE` / `batch-delete`、job-based `:replace`、`documents:sync`、保留 metadata key 拒绝、KB scoped metadata 查询；扩展名、大小、文件数、文本/metadata 限制；同名文件独立目录和独占创建；文本导入 / replace / sync idempotency key 事务内复用；MinIO/S3 source 持久化与本地 cache fallback；JSON/SQLite → PostgreSQL metadata 迁移工具。 | URL/scan/import 类型接入 KB 文档生产流。 | 两个 KB 同名文件上传的 metadata/artifact/query 隔离主路径已覆盖；跨真实生产后端组合仍需集成演练。 |
| Parse-only / artifacts | 单文档 `:parse`、批量 `documents:batch-parse` 聚合 job、job/document 状态流转、fake MinerU/native/docling 成功/失败、request-level `force_reparse`、artifact list/detail、文件型 download、目录型 zip download、文件型 artifact 对象存储预签名 URL、对象存储 URI/prefix 持久化与 restore、跨 KB/缺失文件/路径逃逸/不可信 object URI 拒绝。 | 更细粒度生产 artifact 浏览/预览。 | live MinerU 服务超时/不可用、真实外部服务解析耗时/网络失败仍需更多集成测试；当前 live MinerU 测试为 gated，未配置端点时 skip。 |
...
下一步执行建议：MVP 主链路、Phase 4C document replace、全 KB rebuild、可选 durable worker、共享图谱删除策略 API、运行 job 协作式取消、单文档 delete durable 恢复、PostgreSQL KB metadata 后端、JSON/SQLite → PostgreSQL metadata 迁移工具和 MinIO/S3 source/artifact 持久化均已具备，`:reindex`/`force_*` 已修复为真正重跑 LightRAG pipeline，后续应继续收敛生产 runbook 与集成验证。推荐顺序为：
