...
> - ✅ **基于三段 hash 的增量入库**。`source_hash` 决定要不要重解析（已在 Phase 2 的 parse cache 落实），`parser_hash` 决定 parse 复用还是重跑，`index_hash` 决定 KG/index 是否需要重建；`index_hash` 由 chunker、embedding func/dim、tiktoken model、entity_types、language、extraction prompt 等组合派生。命中且文档已 ready 时 `:build-kg` 直接走 skip 分支并把 job 标 `succeeded`、`result.skipped=true`，**不会触发 LightRAG pipeline**。
...
> - ⏭️ 仍未执行：KB 级 query wrapper、document delete/replace/enable/disable 独立动作、durable worker 恢复、生产 DB migration、共享图谱删除策略；用户隔离、RBAC、审计、配额、限流仍按约定暂不执行。
