...
| `docling` | External Docling content extraction engine | `pdf` `doc` `docx` `ppt` `pptx` `xls` `xlsx` `csv` `md` `html` `xhtml` `png` `jpg` `jpeg` `tiff` `webp` `bmp` |
...
CSV remains available through the explicit `legacy` engine, but production routing should prefer `csv:docling-...` before `*:legacy-...` so Docling can preserve table structure; document preview manifests will include a `preview_table_json` variant for CSV.
...
The `mineru` engine writes the complete artifacts returned by the MinerU service (`content_list.json` + optional `full.md` / `middle.json` / `layout.pdf` / `images/`, etc.) into the `__parsed__/<canonical filename>.mineru_raw/` directory during parsing, and writes `_manifest.json` as the integrity validation file. In local mode it first atomically writes `_pending_task.json` after submission; this records the `task_id`, source hash/size, endpoint, upload filename, and options signature so the same task can be resumed after a timeout, network interruption, or LightRAG restart.
...
- **Avoid duplicate work after timeouts**. A retry reuses the original `task_id` only when the pending record's source, endpoint, upload name, and options all match. Completed tasks download immediately, running tasks keep polling, and only a 404/expired result clears pending state and submits a replacement task.

Lifecycle:

| Operation | Behavior |
|---|---|
| First parse | Submit local task → atomically write `_pending_task.json` → poll within `MINERU_TASK_TIMEOUT_SECONDS` → download artifacts → atomically write `_manifest.json` → delete pending file. |
...
| Re-parse (cache miss, matching pending task) | Preserve `_pending_task.json`, clear other partial artifacts, and resume polling the same `task_id`; do not upload again. |
| Re-parse (cache miss, no matching pending task) | Clear the directory, submit a new task, download, and write manifest. |
| Explicit `force_reparse` | Delete manifest, pending state, and partial artifacts, then force creation of a new task. |
...
Force re-parse (bypass cache and pending-task recovery): set `LIGHTRAG_FORCE_REPARSE_MINERU=true`.
...
3. **Do not exceed the local MinerU backend's `max_concurrent_requests`**: excess tasks queue up. Each task still has its own `MINERU_TASK_TIMEOUT_SECONDS` budget, but end-to-end latency and VLM contention increase.
4. **`MAX_PARALLEL_INSERT` doubles as worker pool size and semaphore cap**: the pipeline creates a `Semaphore(max_parallel_insert)`, and each process worker also takes the semaphore before extraction and ingest. So even if you manually raise the worker count, the actual concurrency cap is still bounded by this value — just tune it directly.
5. **Queue size and backpressure**: the small default `QUEUE_SIZE_INSERT=4` is intentional — the process stage is slow and memory-hungry; when the queue fills, analyze blocks, and backpressure reaches the parse stage, preventing thousands of parsing results from piling up in memory at once.
6. **How changes take effect**: all parameters are passed in via `.env` (or environment variables), read once at `LightRAG` construction; restart the service after changing them.
