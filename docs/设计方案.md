...
>
> **2026-06-10 修订**：按部署前提（[§2.0](#20-部署前提全文档约束)）进一步收敛——SSO/OIDC/SAML 与 per-user 资源配额预留字段从“推迟”改为“明确不做”；`delete_graph_orphans` / `:parse` 的 `auto_index` / 状态机粒度定为固定行为；同轮补齐 KB `visibility` 可见性语义与管理员清空用户租户归属两项残留缺口（见 [§9.3](#93-2026-06-10-范围收敛与本轮补齐)）。
...
| 读取（详情/status/artifact list/preview/doc/job/config/query/graph） | `kb_viewer`+（可由 visibility 隐含，见 §7.5）                                                                                                                                                                                                                                           |
