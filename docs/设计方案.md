...
- 计费/成本配额、token/cost 预算（当前只提供单机 request rate/quota 与并发 job 限额）。
- 单服务器之外的部署模型（本阶段只支持并验收一台服务器）。
- SSO / OIDC / SAML。
- 内置 WebUI / 前端管理台（本轮为 **API-only** 交付，后续由独立前端系统对接已交付 API）。
- 复杂组织树 / 继承策略 / 跨租户共享 / tenant admin 自助。
- 复杂 service key 组织继承策略。
- artifact 自定义 action policy（per-artifact-type policy 与 storage URI masking 已交付）。
...
| service/scoped API key：创建/列表/撤销/rotation、hash 存储、`kb_roles` scope、即时失效、`expires_at` 过期(鉴权时校验) | ✅ | 不允许建 KB / 成 super admin；raw key 仅创建/轮换响应返回一次 |
...
| artifact 动作级权限 + per-type policy + storage URI masking | ✅ | `LIGHTRAG_ENTERPRISE_ARTIFACT_DOWNLOAD_MIN_ROLE` 设置全局下载/预签名最低角色；`LIGHTRAG_ENTERPRISE_ARTIFACT_DOWNLOAD_POLICY` 可按 artifact type 覆盖；企业模式默认 mask local/object storage URI |
| 请求限流 / 配额（默认关闭，单机内存固定窗口，user/tenant/key 分桶，429+Retry-After+审计） | ✅ | token/cost 成本配额不属于本阶段 |
...
| artifact `:download` / `:download-url` | `LIGHTRAG_ENTERPRISE_ARTIFACT_DOWNLOAD_MIN_ROLE`（默认 `kb_viewer`，可升级）；可被 `LIGHTRAG_ENTERPRISE_ARTIFACT_DOWNLOAD_POLICY` 按 artifact type 覆盖 |
...
>
> **2026-06-09 补齐（单机部署闭环）**：`/metrics` HTTP counter/histogram、告警规则 / SLO / Grafana dashboard、单机恢复 smoke 与 hard-delete drill 脚本、MinerU 超时/网络中断回归、service key rotation、artifact per-type policy + storage URI masking、注册失败锁定、`:batch-parse` in-process 并发、单文档 delete 幂等策略指纹、URL/import/scan auto-parse 幂等回归均已实现并通过焦点测试。
...
| 请求限流 / 配额 + 并发 jobs 配额 + 登录/注册失败锁定 | ✅ | 单服务器内存固定窗口和 per-principal/tenant job 限额已完成；token/cost 成本配额属 §9.2 |
...
| 计费 / token/cost 成本配额 | 当前只有 request rate/quota 与并发 job 限额；token/cost 预算、计费结算后续再做 |
| 复杂组织层级 / 跨租户共享 / 继承策略 | 多租户高级治理 |
| 复杂 service key 组织继承策略 | 当前 service key 只按显式 `kb_roles` scope 授权 |
| artifact 自定义 action policy | 当前支持全局最低角色 + per-artifact-type 最低角色 policy + storage URI masking；更复杂 action policy 后续再做 |
...
| token/cost 成本不可控 | 无法按 LLM token 或成本做预算拦截 | 暂不纳入本阶段；仅提供 request quota 与并发 job 限额 |
