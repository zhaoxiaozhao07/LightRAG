...
> 状态以各源文档 2026-06-08 自述 + 2026-06-09 单机闭环补齐与测试记录为准。结构化清单（待办 P0–P4、企业能力补齐清单、企业验收清单）当时均 100% 勾选；2026-06-09 已补齐单机部署范围内 `/metrics` HTTP 指标、service key rotation、artifact per-type policy/masking、注册失败锁定与若干幂等/并发回归测试。剩余 🟡/⏳ 均为真实环境演练或后续平台能力。
...
- metrics 告警 / SLO / dashboard（HTTP 计数与延迟 histogram 已在单机进程内暴露）。
- SSO / OIDC / SAML。
...
| 登录/注册失败锁定（企业 `/login` 与 `/auth/register`,默认开启,per-username 窗口计数 + 锁定 + 审计,可配/可关） | ✅ | 单进程内存；分布式协调 ⏳ |
| Prometheus `/metrics`（KB/doc/job/audit gauge + process-local HTTP counter/histogram，`combined_auth` 保护） | ✅ | 告警规则 / SLO / dashboard ⏳ |
...
>
> **2026-06-09 补齐（单机部署闭环）**：`/metrics` HTTP counter/histogram、service key rotation、artifact per-type policy + storage URI masking、注册失败锁定、`:batch-parse` in-process 并发、单文档 delete 幂等策略指纹、URL/import/scan auto-parse 幂等回归均已实现并通过焦点测试。

### 9.1 做了但没做完（🟡 — 代码已落地，缺完整功能 / 真实环境验证）

| 项 | 现状 | 缺口 |
|---|---|---|
| Prometheus `/metrics` | KB/doc/job/audit gauge + 单进程 HTTP counter/histogram | 告警规则、SLO、dashboard |
| 请求限流 / 配额 + 并发 jobs 配额 + 登录/注册失败锁定 | 默认关闭或默认开启的单进程内存计数 | 分布式共享计数、token/cost 配额 |
| 租户 / 组织治理 | 扁平 membership + tenant ACL | 组织树、继承策略、跨租户共享、tenant admin 自助 |
| service/scoped API key | 创建/列表/撤销/rotation/scope/`expires_at` 过期 | 租户继承 |
| artifact 动作级权限 | role 阶梯、per-type policy、storage URI masking 可配 | 自定义 action policy |
