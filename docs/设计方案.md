...
| 租户实体 CRUD + 总览（`/admin/tenants` 创建/列表/详情/`PATCH`、`/admin/tenants/{id}/kbs`） | ✅ | super admin 管理租户实体（name/description/status）；详情返回 `member_count`/`kb_count`；`tenant_id` 在用户/KB 上仍为自由赋值（不强制存在性），租户实体用于显式治理与总览 |
