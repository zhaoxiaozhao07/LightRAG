...
| 跨 KB 合并问答 `:query` / `:retrieve`（scatter-gather） | ✅ | 各 KB 经自身实例 `aquery_data` 检索、chunk 打 `kb_id` 标注；共享 reranker 统一重排合并（`process_chunks_unified`）、统一重新编号引用（`generate_reference_list_from_chunks`）；单次 `role_llm_funcs["query"]` 合成；KB 隔离不变，全套模型为共享部署服务，跨库用统一查询参数 |
...
| 跨 KB `POST /kbs:query`、`/kbs:retrieve` | `kb_ids` 每个成员均需 `kb_viewer`+（handler 自鉴权，中央中间件不覆盖 collection 级 `:query` 路径）；`bypass` 不支持 |
