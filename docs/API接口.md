...
- **客户端中断（停止生成）行为**：前端断开连接（关闭 socket / abort 请求）时，服务端会停止该请求的后端工作，尽量不浪费检索与 LLM 算力（断开检测为轮询式，间隔约 0.5s，最坏情况有相应的检测延迟；取消后的资源清理最多等待 5s，超时会记录告警并弃置该后台任务）：
  ...
  - 该机制对 `/kbs/{kb_id}/query`、`/query/stream`、`/query/data`、`/retrieve`、`/kbs:query`、`/kbs:query/stream`、`/kbs:retrieve` 以及全局 `/query` 系列均生效。若底层传输无法上报断开（如某些不感知 `http.disconnect` 的反代），则退化为旧行为（跑完丢弃），不影响正常输出。
...
支持的 `mode`：`local` / `global` / `hybrid` / `naive` / `mix` / `bypass`。`/query/stream` 与 KB scoped stream 一样返回 NDJSON；当请求体 `stream=false` 或底层返回非流式结果时，会返回单行完整 NDJSON。请求体支持 `bilingual` 字段启用双语双路检索（无 KB config 层，按"请求体 > 部署默认值"解析，行为同 §8.2）。客户端断开（停止生成）行为见 §8 约束「客户端中断（停止生成）行为」：`/query`、`/query/stream`、`/query/data` 同样支持断开即取消后端工作并返回 `499`。
...
# 租户管理员(tenant_admin/tenant_owner)对本租户所有 KB(含成员私有 KB)的监察保底角色；
# 默认 kb_viewer(只读), 可设 kb_editor / kb_admin 以获得读写/管理能力; kb_owner 不允许; 非法值回退 kb_viewer
