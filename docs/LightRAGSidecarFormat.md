...
| `llm_analyze_result.equation` | String: the **canonicalized** LaTeX equation output by the LLM (outer `$ / \[ \] / equation` environment, Unicode converted to LaTeX, no leading/trailing `$` delimiters); this is the string actually used by the downstream multimodal chunk. **Fallback semantics**: when the LLM returns JSON missing or emptying the `equation` field (typical with complex formulas due to token truncation / field-name drift), the analyze stage falls back to this item's `content` (the raw LaTeX) as the `equation` value, `status` stays `success`, `degraded=true` is recorded, and `message` records "fell back to sidecar raw latex" with a WARNING log line — so a single equation's LLM hiccup does not fail-fast and bring down the whole document. The fallback result is NOT written to the analysis cache; a matching historical bad cache entry is evicted, so a re-run gets another shot at a clean LLM response. If the full raw LaTeX still exceeds `MAX_EXTRACT_INPUT_TOKENS`, LightRAG keeps the original equation in the sidecar/body but omits that equation's multimodal chunk rather than failing the whole document build. Only when both the `equation` field is missing AND `content` is empty does the item escalate to `failure` (in practice this is intercepted upstream by the `missing equation content` guard). |
...
|---|---|---|
| `success` | The model returns valid JSON and all required fields are present; **includes degraded fallback**: when the equation's `equation` field is missing/invalid but sidecar raw LaTeX is available, the analyze stage falls back to the raw LaTeX and continues — `status` remains `success` | Drawing: `name / type / description`; Table: `name / description`; Equation: `name / description / equation / degraded` (on fallback, `equation` is taken from sidecar `content`, `degraded=true`, and `message` records the reason; clean equations use `degraded=false`) |
...
| `failure` | Required fields (except equation's `equation`) missing, JSON still invalid after repair, the VLM/EXTRACT role is not configured while the corresponding modality is enabled, or the model invocation throws an exception; an equation also escalates to `failure` when its `equation` field is missing **and** sidecar `content` is also empty | `message` records the diagnostic |

Additional notes:

- `analyze_time` is epoch seconds and is present for every status;
- `message` when `status="success"`: empty in the normal case; **non-empty under degraded fallback**, where it records `"equation field missing or invalid in LLM response; fell back to sidecar raw latex"` so operators can filter degraded items via `degraded=true` or `message != ""`;
- Items for enabled modalities are recomputed on each `analyze_multimodal` run, and the current run overwrites any prior `llm_analyze_result` (`success`, `skipped`, or `failure`). This allows operators to fix VLM/EXTRACT configuration and retry without manually clearing stale sidecar results. LLM calls still use the analysis cache: if the cache key matches, the provider is not called and semantic fields usually remain the same, though runtime fields such as `analyze_time` are rewritten. A cache miss, for example after changing the effective role model/binding/host, prompt inputs, or image metadata, can produce different saved content. **Degraded fallback results are not cached, and a matching historical degraded cache entry is evicted**, so a re-run re-attempts a clean LLM response.
