...
async def test_claim_next_worker_job_is_single_winner(tmp_path: Path):
    ...
    await store.create_job(_job("kb_a", "job_1", job_type="parse"))
...
async def test_claim_respects_job_type_filter(tmp_path: Path):
    store = await _make_store(tmp_path)
    await store.create_job(_job("kb_a", "job_build", job_type="build_kg"))
...
async def test_claim_grace_window_excludes_fresh_jobs(tmp_path: Path):
    ...
    await store.create_job(_job("kb_a", "fresh", job_type="parse"))
...
async def test_claim_orders_oldest_first(tmp_path: Path):
    ...
    await store.create_job(
        _job("kb_a", "newer", queued_at="2026-05-29T10:00:00+00:00")
    )
    await store.create_job(
        _job("kb_a", "older", queued_at="2026-05-29T09:00:00+00:00")
    )
    claimed = await store.claim_next_worker_job(
        job_types=["parse"], max_queued_at=None
    )
...
async def test_worker_poll_once_dispatches_to_executor(tmp_path: Path):
    ...
    await store.create_job(_job(record.id, "job_exec", job_type="parse"))
...
async def test_worker_consumes_retried_job(tmp_path: Path):
    ...
    await store.create_job(_job(record.id, "job_retry", job_type="parse"))
...
async def test_worker_executor_error_marks_job_failed(tmp_path: Path):
    ...
    job_service = JobService(kb_service, store)
    await store.create_job(_job(record.id, "job_bad", job_type="parse"))
...
async def test_recovery_leaves_resumable_queued_jobs(tmp_path: Path):
    ...
    await store.create_job(_job("kb_r", "queued_parse", job_type="parse"))
    await store.create_job(_job("kb_r", "queued_delete", job_type="delete"))
    await store.create_job(_job("kb_r", "running_parse", job_type="parse"))
